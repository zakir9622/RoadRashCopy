...
def build_skinned_mesh(arm, suit, skin, helm, visor, scale=1.0):
    ...
    bone_cyl("hips", 0.11, suit)
    bone_cyl("spine", 0.12, suit)
    bone_cyl("chest", 0.14, suit)
    bone_cyl("neck", 0.045, skin)
    h, t = _world_bone(arm, "head")
    parts.append(_sphere_at("mesh_head", (h + t) * 0.5, 0.10 * s, skin, 16))
    parts.append(_sphere_at("helmet", (h + t) * 0.5 + Vector((0, 0.02 * s, 0.02 * s)), 0.13 * s, helm, 16))
    visor_pos = (h + t) * 0.5 + Vector((0, 0.09 * s, 0.01 * s))
    bpy.ops.mesh.primitive_cube_add(size=1, location=visor_pos)
    vis = bpy.context.active_object
    vis.name = "visor"
    vis.scale = (0.09 * s, 0.02 * s, 0.045 * s)
    bpy.ops.object.transform_apply(scale=True)
    vis.rotation_euler = (_rad(18), 0, 0)
    vis.data.materials.append(visor)
    parts.append(vis)

    for side in ("l", "r"):
        bone_cyl(f"shoulder_{side}", 0.055, suit)
        bone_cyl(f"upper_arm_{side}", 0.048, suit)
        bone_cyl(f"forearm_{side}", 0.042, suit)
        hh, ht = _world_bone(arm, f"hand_{side}")
        parts.append(_sphere_at(f"glove_{side}", (hh + ht) * 0.5, 0.045 * s, suit, 10))
        bone_cyl(f"thigh_{side}", 0.07, suit)
        bone_cyl(f"shin_{side}", 0.055, suit)
        fh, ft = _world_bone(arm, f"foot_{side}")
        parts.append(_cyl_between(f"boot_{side}", fh, ft, 0.05 * s, suit, 10))
