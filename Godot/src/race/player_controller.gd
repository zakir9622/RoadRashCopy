...
extends Node
## Reads input and writes it onto the player Rider. Touch controls live in the
## HUD; this handles keyboard/gamepad and the shared attack plumbing.

var rider: Rider
var get_opponents: Callable = Callable()

# Touch intent written by HUD buttons.
...
func _physics_process(_delta: float) -> void:
	...
	rider.in_throttle = clampf(throttle - brake, 0.0, 1.0)
	...
	if Input.is_action_just_pressed("kick") or touch_kick:
		# Kick whichever side has someone; default right.
		var side := 1.0
		for opponent in opponents:
			var other := opponent as Rider
			if other != rider and absf(other.distance - rider.distance) < 2.0:
